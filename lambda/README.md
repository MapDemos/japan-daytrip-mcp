...
4. Settings:
   - Function name: `japan-daytrip-claude-proxy`
...
2. Copy the **API endpoint** URL (looks like: `https://abc123.execute-api.us-east-1.amazonaws.com/default/japan-daytrip-claude-proxy`)

#### Step 7: Update Frontend Config

Edit `config.js`:
```javascript
CLAUDE_API_PROXY: 'https://YOUR-API-ID.execute-api.us-east-1.amazonaws.com/default/japan-daytrip-claude-proxy'
```
...
Resources:
  ClaudeProxyFunction:
    ...
    Properties:
      FunctionName: japan-daytrip-claude-proxy
...
View Lambda logs:
```bash
aws logs tail /aws/lambda/japan-daytrip-claude-proxy --follow
```
...
aws apigatewayv2 get-apis --query 'Items[?Name==`japan-daytrip-claude-proxy`].ApiId' --output text
aws apigatewayv2 delete-api --api-id YOUR-API-ID

# Delete IAM role
aws iam detach-role-policy \
  --role-name japan-daytrip-lambda-role \
  --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole
aws iam delete-role --role-name japan-daytrip-lambda-role
```
